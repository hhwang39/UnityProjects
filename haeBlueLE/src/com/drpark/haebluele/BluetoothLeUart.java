...
public class BluetoothLeUart extends BluetoothGattCallback implements BluetoothAdapter.LeScanCallback {
    ...
    // UUIDs for the Device Information service and associated characeristics. Arudino doesn't have it
/*    public static UUID DIS_UUID       = UUID.fromString("0000180a-0000-1000-8000-00805f9b34fb");
    ...
    public final String ARDUNIOADDR = "EE:B6:3A:45:D0:A4";
    // Internal UART state.
    ...
    // Device Information state. not supported by arduino
...
/*    public String getDeviceInfo() {
    ...
    public void send(byte[] data) {
        if (tx == null || data == null || data.length == 0) {
        	UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr", "null value");
            // Do nothing if there is no connection or message to send.
            return;
        }
        UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr", "Writing");
    ...
    public void send(String data) {
    	UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr", "Writing" + data);
    ...
    //hardcode some functions to make it work %NICK
    ...
    public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
        ...
        if (newState == BluetoothGatt.STATE_CONNECTED) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
            	UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr", "Successful with " + (gatt == null ? "NO1 GATT":"YES GATT"));
                // Connected to device, start discovering services.
            	//gatt.discoverServices();
                if (!gatt.discoverServices()) {
                	UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr", "Not so succesful");
                    // Error starting service discovery.
                    connectFailure();
                }/* else {
                	UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr", "connecting service");
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        //only for testing
/*        List<BluetoothGattService> a = gatt.getServices();
        for (int i = 0; i < a.size(); ++i) {
        	List<BluetoothGattCharacteristic> b = a.get(i).getCharacteristics();
        	for (int j = 0; j < b.size(); ++j) {
        		UnityPlayer.UnitySendMessage("BluetoothLE", "AddDevice", b.get(j).getUuid().toString());
        	}
        	
        }*/
        //again arduino doesn't have it and gives error
       /* UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr","rx = " + (rx == null ? "is NULL":"is NOT NULL"));
        ...
        UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr", "disManuf1 " + (disManuf == null ? "NULL":"NOT NULL"));
        ...
        UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr", "disManuf " + (disManuf == null ? "NULL":"NOT NULL"));
        ...
        UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr", "service discovered");
    ...
    public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
    	UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr", "reading ....");
    ...
    public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        //UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr", "on char changed");
        if (status == BluetoothGatt.GATT_SUCCESS) {
    ...
    //hardcode again to make sure that function is called
    @Override
    public void onLeScan(BluetoothDevice device, int rssi, byte[] scanRecord) {
        ...
        if (connectFirst && device.getAddress().equals(ARDUNIOADDR)) {
            // Stop scanning for devices.
            stopScan();
            // Prevent connections to future found devices.
            connectFirst = false;
            // Connect to device.
            UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr", "Let me connect to the device: " +
            							device.getName());
            bDevice = device;
            UnityPlayer.UnitySendMessage("BluetoothLE", "notifiedDoneScanning", "");
        }
    }
    //need to connect seperatly 4.3 bugs
    public void connectUART() {
        gatt = bDevice.connectGatt(context, true, this);
        //gatt.connect();
    }
    ...
    private void notifyOnConnected(BluetoothLeUart uart) {
    	UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr", "Connected from LE");
    ...
    private void notifyOnDeviceFound(BluetoothDevice device) {
    	UnityPlayer.UnitySendMessage("BluetoothLE", "setSystemStr", "Device Found");
        for (Callback cb : callbacks.keySet()) {
            if (cb != null) {
